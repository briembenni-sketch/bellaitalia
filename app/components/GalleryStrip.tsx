...
export default function GalleryStrip() {
  ...
  return (
    <div className="relative overflow-hidden group" aria-label="Myndir frá Ítalíu">
      <div className="pointer-events-none absolute inset-y-0 left-0 w-10 md:w-28 bg-gradient-to-r from-ink to-transparent z-10" />
      <div className="pointer-events-none absolute inset-y-0 right-0 w-10 md:w-28 bg-gradient-to-l from-ink to-transparent z-10" />
      <div className="flex gap-3 md:gap-4 w-max animate-marquee group-hover:[animation-play-state:paused]">
        {items.map((img, i) => (
          <div
            key={`${img.src}-${i}`}
            className={`relative h-44 sm:h-56 md:h-72 shrink-0 overflow-hidden rounded-2xl md:rounded-3xl bg-white/8 ${
