...
export default function VillaCard({ villa, ctaHref = "/villur#fyrirspurn", large = false }: Props) {
  return (
    <article className="group flex flex-col h-full rounded-3xl bg-white border border-ink/5 overflow-hidden hover:border-ink/15 transition-colors">
      <div className={`relative overflow-hidden bg-mist ${large ? "aspect-[16/10]" : "aspect-[4/3]"}`}>
        <Image
          src={villa.image}
          alt={villa.imageAlt}
          fill
      ...
      <div className={`flex flex-col flex-1 ${large ? "p-6 md:p-8" : "p-5 md:p-6"}`}>
        <p className={`text-ink/60 leading-relaxed ${large ? "text-[15px] md:text-base" : "text-[15px] line-clamp-3"}`}>{villa.text}</p>
        <ul className="mt-4 flex flex-wrap gap-2">
          {villa.features.map((f) => (
            <li key={f} className="rounded-full bg-mist px-3 py-1 text-xs text-ink/70">
              {f}
            </li>
          ))}
        </ul>
        <div className="mt-auto pt-5 flex items-center justify-between gap-3">
          <span className="text-sm text-ink/50">Verð eftir fyrirspurn</span>
          <Link
            href={ctaHref}
            className="inline-flex items-center gap-2 rounded-full border border-ink/15 px-4 py-2 text-sm font-medium hover:bg-ink hover:text-white transition-colors"
          >
